    ...
    Module[{result, dim, dimStr, massName, particleName, mixingMatrix, selfEnergyFunction,
           ...
           If[dim > 1,
              If[(SARAH`UseHiggs2LoopMSSM === True ||
                  ...
                  FlexibleSUSY`UseHiggs3LoopMSSM === True) && FlexibleSUSY`UseSARAH2Loop === False &&
             ...
             If[FlexibleSUSY`UseSARAH2Loop === True &&
               MemberQ[{SARAH`HiggsBoson, SARAH`PseudoScalar}, particle],
               calcHigherLoopHiggsContributions = calcHigherLoopHiggsContributions <> CalcEffPot2L[particle];
...
" <> If[FlexibleSUSY`UseSARAH2Loop === True,
   "auto model_gl = *this;\nmodel_gl.enter_gaugeless_limit();\n" <>
   "self_energy_2l = Re(model_gl.self_energy_" <> CConversion`ToValidCSymbolString[particle] <> "_2loop(old_" <>ToValidCSymbolString[TreeMasses`GetMass[particle]] <> "(0)" <> "));\n" <>
   If[FlexibleSUSY`UseConsistentEWSBSolution === True,
      "if(ewsb_solve_consistently) {\n" <>
      "  self_energy_2l += Re(model_gl.self_energy_shift1L_" <> CConversion`ToValidCSymbolString[particle] <> "_2loop(old_" <>ToValidCSymbolString[TreeMasses`GetMass[particle]] <> "(0)" <> "));\n" <>
      "}\n",""]
...
DoSlowDiagonalization[particle_Symbol, tadpole_] :=
    Module[{result, dim, dimStr, massName, inputMomenta, outputMomenta,
           ...
           If[dim > 1 && FlexibleSUSY`UseSARAH2Loop === False &&
